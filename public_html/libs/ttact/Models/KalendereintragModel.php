...
class KalendereintragModel extends Model
{
    ...
    public static function findByStartEndMitarbeiterLohnrelevant(\ttact\Database $db, \DateTimeInterface $inclusive_start, \DateTimeInterface $inclusive_end, int $mitarbeiter)
    {
        ...
        $selection = $db->getRowsQuery("SELECT * FROM kalendereintrag WHERE mitarbeiter_id = '$mitarbeiter' AND type IN ('krank_bezahlt', 'feiertag_bezahlt', 'urlaub_bezahlt') AND ((von >= '$a' AND von <= '$b') OR (bis >= '$a' AND bis <= '$b') OR (von < '$a' AND bis > '$b')) ORDER BY von");
