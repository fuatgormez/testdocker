...
class KundenkonditionModel extends Model
{
    ...
    public static function getKundeAbteulengPries(\ttact\Database $db,string $kunde_id, string $abteilung_id)
    {
        $return = $kunde_id." --".$abteilung_id ;

        $all = $db->getRowsQuery("SELECT * FROM kundenkondition WHERE kunde_id = '".$kunde_id."' AND abteilung_id = '".$abteilung_id."'  ORDER BY kundenkondition_id DESC LIMIT 1");
        $return = new self($db, $all[0]);
