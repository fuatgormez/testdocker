...
{{$title="Benutzer | bearbeiten" scope=parent}}

<div class="row mb-4">
    <div class="col">
        <div class="content-box">
            <h3 class="content-box-title">Benutzer <strong>{{$smarty_vars.values.benutzername|default:"???"}}</strong></h3>
            ...
            <form action="/benutzer/bearbeiten/{{$smarty_vars.values.id|default:"???"}}" method="post">
                <div class="row">
                            ...
                            <select class="selectable selectable_multiple form-control" id="kundenbeschraenkungen" tabindex="-1" name="kundenbeschraenkungen[]" data-placeholder=" keine Beschränkung" multiple>
                                {{if isset($smarty_vars.kundenliste)}}
                                    {{foreach from=$smarty_vars.kundenliste item='row'}}
                                        <option value="{{$row.id}}" {{if isset($smarty_vars.values.kundenbeschraenkungen)}} {{if in_array($row.id, $smarty_vars.values.kundenbeschraenkungen)}} selected{{/if}}{{/if}}>{{$row.kundennummer}} - {{$row.name}}</option>
                                    {{/foreach}}
                                {{/if}}
                            </select>
                        </div>
                    ...
                    <div class="form-group col-12 col-sm-6">
                        ...
                        <div class="input-group">
                            ...
                            <select class="form-control selectable" id="aktiv" tabindex="-1" name="aktiv">
                                <option value="1"{{if isset($smarty_vars.values.aktiv)}}{{if $smarty_vars.values.aktiv == 'ja'}} selected{{/if}}{{/if}}>ja</option>
                                <option value="0"{{if isset($smarty_vars.values.aktiv)}}{{if $smarty_vars.values.aktiv == 'nein'}} selected{{/if}}{{/if}}>nein</option>
                            </select>
                        </div>
                    </div>
                </div>
